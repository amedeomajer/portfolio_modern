...
const Experience = () => {
  return (
    <section id="experience" className="section flex items-center justify-center py-20 px-4">
      <motion.div
      ...
      >
        ...
        >
          <h2 className="text-2xl md:text-3xl font-bold mb-8">{cvData.work.title}</h2>
          ...
          <div className="space-y-8">
            {[...cvData.work.positions].reverse().map((position, index) => (
              ...
              >
                ...
                <div className="mb-4">
                  <h3 className="text-xl md:text-2xl font-semibold">{position.company}</h3>
                    ...
                    {position.impact.map((item, i) => (
                      <li key={i} className="text-text-muted text-sm flex items-start gap-2">
                        ...
                        <span>
                          {item.description}
                          {item.note && (
                            <em className="block text-white/40 text-xs mt-1">{item.note}</em>
                          )}
                        </span>
                      </li>
                    ))}
        ...
        >
          <h2 className="text-2xl md:text-3xl font-bold mb-6">{cvData.education.title}</h2>

          <div className="relative pl-6 border-l-2 border-glass-border">
            <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-bg-dark border-2 border-glass-border" />

            <p className="text-text-muted mb-4">{cvData.education.description}</p>
            <p className="text-text-muted mb-4">{cvData.education.additionalInfo}</p>
